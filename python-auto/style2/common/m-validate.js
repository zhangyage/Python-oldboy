...
var MValify = {
		...
		username: function(str, result, id){
			str = str.replace(/^\s\s*/, '').replace(/\s\s*$/, '');//去掉收尾空格
			if(!str){
				AlertDialog.show("用户名不能为空", id, -20, 100);
				return false;
			}else if(/[\u4E00-\u9FA5]/i.test(str)){
				AlertDialog.show("用户名不能含有中文", id, -20, 100);
				return false;
			}else if(!isNaN(str)){
				AlertDialog.show("用户名不能为纯数字", id, -20, 100);
				return false;
			}else if(str.length < 6 || str.length > 16){
				AlertDialog.show("用户名长度应该为6到16位", id, -20, 100);
				return false;
			}else{
				//如果验证用户名格式通过，验证是否重复
				$.ajax({
					...
					success: function(data){
						if(data["msg"] == "success"){
							...
							AlertDialog.show(data["msg"], id, -20, 100);
							result = false;
						}
					},
					error: function(){
		...
		password: function(str,id){
			if(!str){
				AlertDialog.show("密码长度应该为6~16位", id, -20, 100);
				return false;
			}
		...
		rePass: function(str, str1, id){
			if(!str1){
				AlertDialog.show("密码不能为空", id, -20, 100);
				return false;
			}
			if(str != str1){
				AlertDialog.show("两次输入密码不一致", id, -20, 100);
				return false;
			}else{
				AlertDialog.hide();
				return true;
		...
		phone: function(phone, id){
			...
			if(!phone){
				AlertDialog.show("手机号不能为空", id, 20, 100);
				return false;
			}
			if(isPhoneReg.exec(phone)){
				...
				AlertDialog.show("手机号不正确", id, 20, 100);
				return false;
			}
		},
		//验证手机验证码
		...
		email: function(str, id){
			var $inputt = $("#" + id).parent().children(".inputf");
			if(!str){
				AlertDialog.show("请输入电子邮箱", id, 0, 50);
				return false;
			}
			...
			if(isEmailReg.test(str)){
				...
				AlertDialog.show("请输入正确的电子邮箱", id, 0, 50);
				return false;
			}
		},
		//真实姓名
		...
		cardCode: function(code, id){
			if(!code){
				AlertDialog.show("证件号码不能为空", id, 0, 50);
				return false;
			}else{
				var num = code.toUpperCase();  
				//身份证号码为15位或者18位，15位时全为数字，18位前17位为数字，最后一位是校验位，可能为数字或字符X。
				if (!(/(^\d{15}$)|(^\d{17}([0-9]|X)$)/.test(num))) {
					AlertDialog.show("身份证号码不正确", id, 0, 50);
					return false;
				}
